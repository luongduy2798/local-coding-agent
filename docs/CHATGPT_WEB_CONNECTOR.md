...
Runtime publish catalog cố định 34 tool (`catalog_version=17`). `task_plan` đã hợp nhất progress actions, `verify_changes` đã hợp nhất impacted/full verification, và `notes` không còn model-facing. Khi catalog thay đổi:
