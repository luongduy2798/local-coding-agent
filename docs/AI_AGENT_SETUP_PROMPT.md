...
- Xác minh runtime fixed 34-tool/catalog_version=17, advisory task orchestration, Persistent Workspace Memory Phase 1 và multi-workspace registry.
...
9. Hướng dẫn tạo ChatGPT connector với `No auth`, refresh connector nếu nó
   từng dùng legacy, mở chat mới và gọi `lca_status` để xác minh `catalog_version=17`.
```
