...
function serverInstructions(policy) {
  return [
    ...
    "MUTATION: before apply_patch, open the first task with explicit primary_workspace_id or a later task with the conversation_workspace_token already pinned by the first task. Mutation never auto-creates a task or chooses a fallback workspace. Use apply_patch with expected_version for every source create/update/delete/rename, including cross-workspace batches. A transaction reported in_doubt blocks further mutation until recovery. Shell, process, verification, or Git changes to tracked source are unmanaged integrity violations: they cannot be adopted, cannot produce an exact task journal, and block a complete close until the source is restored and reapplied through apply_patch.",
    "REVIEW: review_diff is conditional, not mandatory after every mutation. scope=task is the default and requires the current task's exact journaled change set; missing or unmanaged journal evidence returns INCOMPLETE and never falls back to workspace review. Use scope=workspace only when the user explicitly requests review of every staged, unstaged and untracked Git change under cwd. Clean mechanical mutations may close directly from apply_patch transaction evidence without review_diff; set apply_patch.close_on_success=true only on the final known-safe patch to run the same task_close completion guard internally. Use git only for raw status, diff or history inspection, and change_history action=diff for one journaled change ID.",
