...
- `response_mode=auto` giữ payload compact cho quick/normal và đầy đủ cho complex; chỉ dùng `full`/`diagnostic` khi quyết định tiếp theo thật sự cần evidence chi tiết. Compact response không được làm mất audit, journal, Undo/Reapply hoặc dữ liệu durable phía server.
