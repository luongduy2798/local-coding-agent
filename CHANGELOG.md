...
- Added `auto|minimal|compact|full|diagnostic` response shaping. Task lifecycle and patch responses now default to compact payloads for every profile, complex review evidence remains full by default, and durable detail remains server-side. Compact Memory returns one bounded brief instead of duplicating full item metadata; `task_open.focused_target` can return a known file snippet and content version without a separate read call.
...
- Restored the mechanical quick-edit fast path (`task_open` → `apply_patch` → `task_close`), added optional `apply_patch.close_on_success` that invokes the same guarded task-close handler internally, and made `review_diff` conditional and task-scoped by default: managed findings use exact journal before/after snapshots, Git evidence is restricted to task paths, and pre-existing dirty changes stay outside the task review. Added explicit `scope=workspace` for reviewing every staged, unstaged and untracked Git change under `cwd`.
...
- Rebuilt the VS Code extension as a four-tab Control Center (Overview, Workspaces, Tasks, Changes), while keeping task plans/model thinking out of the UI. It lists the complete registry, selects and orders the current VS Code window's repository first instead of exposing an aggregate workspace sentinel, monitors safe tool/verification/process metadata from rotating `audit.log`, and reconnects Polling back to Live.
